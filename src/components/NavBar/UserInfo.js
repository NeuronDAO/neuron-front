...
export default function UserAutch (props) {
    ...
    if(props.isAutch !== false) {            
        const handleClick = (event) => {setAnchorEl(event.currentTarget);}; // Обработчик на клик по аватарке         
        const handleClose = () => {setAnchorEl(null);};  // Обработчик на по ссылке (или по области экрана)
        return(
            <>
              {/* LINK GO TO CREATE PROJECT */}
              <Button  variant="outlined" component={Link} to="/create-bounty" >Create new</Button>
             ...
             {/* DROP DOWN ACTION AND USER AVATAR */}
              ...
              <Menu
                ...
                {/* DROP DOWN ITEMS  */}
                      <MenuItem onClick={handleClose} component={Link} to="/my/profile" className={classes.itemLink}>Profile</MenuItem>
                      <MenuItem onClick={handleClose} component={Link} to="/my/setting" className={classes.itemLink}>Account setting</MenuItem>
                      <MenuItem onClick={handleClose} component={Link} className={classes.itemLink}>Logout</MenuItem>
                  </Menu>
              </>
          )
      } 
